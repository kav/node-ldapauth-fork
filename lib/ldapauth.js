...
LdapAuth.prototype._findGroups = function(user, callback) {
  ...
  var groupSearchFilter = self.opts.groupSearchFilter;
  if (typeof groupSearchFilter === 'string') {
    var groupSearchString = groupSearchFilter;
    groupSearchFilter = function(user) {
      return groupSearchString.replace(/{{dn}}/g, user[self.opts.groupDnProperty]);
    };
  }
  var searchFilter = groupSearchFilter(user);
